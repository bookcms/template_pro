...
<head>
	...
	<link rel="canonical" href="{$site_config.site_domain}{:url('/author/' . urlencode($author))}"/>
	...
	<style mip-custom>
		...
		mip-form #mip-form-cross{right: initial!important;}mip-img{float: left;}
		.zz52{line-height:30px!important;}.footerad{height:80px;}body:before{position: unset!important;}
	</style>
</head>
<body>
...
<div class="container">
	...
	<div class="content book" id="fengtui">
		...
		<div class="bookbox"><div class="p10"><span class="num"><a data-type="mip" data-title="{$novel.Title}" href="{:url("/book/" . $novel['PrimaryId'],"","html",true)}"><mip-img layout="fixed" width="90" height="120" src="{$novel.LocalImage | down_image}" alt="{$novel.Title}"></mip-img></a></span><div class="bookinfo zz52"><h4 class="bookname"><a hdata-type="mip" data-title="{$novel.Title}" href="{:url("/book/" . $novel['PrimaryId'],"","html",true)}">{$novel.Title}</a></h4><div class="author">作者：{$author}</div><div class="author">分类：{$novel.Cid | show_cid_name}</div><div class="cat"><span>更新到：</span><a href="{:url("/chapter/" . $novel.PrimaryId . "/" . get_last_offset_sort($novel.LastChapterSort,$novel['SourceList'],$novel['LastChapterFlag']),"","html",true)}">{$novel.LastChapterTitle}</a></div><div class="update"><span>简介：</span>{$novel.Introduction}</div></div><div class="delbutton"><a class="del_but" data-type="mip" data-title="{$novel.Title}" href="{:url("/book/" . $novel['PrimaryId'],"","html",true)}">阅读</a></div></div></div>
	    {/foreach}
	</div>
	<div class="clear"></div>
</div>
