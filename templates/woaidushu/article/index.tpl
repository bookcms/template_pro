...
<link rel="canonical" href="{:url("/book/" . $novel['PrimaryId'],array(),"html",true)}"/>
...
<div class="headerpc hidden-xs">
	...
	<div class="header_search">
		<mip-form method="get" url="/search" clear>
...
<div class="container">
	<div class="content">
		<ol class="breadcrumb">
            <li><a data-type="mip" href="{$site_config.site_domain}" data-title="{$site_config.site_name}">首页</a></li>
            <li><a data-type="mip" href="{:url("/classify/" . $novel.category.alias,"","",true)}" data-title="{$novel.Cid | show_cid_name }小说">{$novel.Cid | show_cid_name }</a></li>
            <li class="active">{$novel.Title}</li>
        </ol>
		<div class="book pt10">
			<div class="bookcover hidden-xs">
				<mip-img class="thumbnail" layout="fixed" width="140" height="180" popup src="{$novel.LocalImage | down_image}" alt="{$novel.Title}"></mip-img>
			</div>
			<div class="bookinfo">
				<h1 class="booktitle">{$novel.Title}</h1>
				<div class="booktag">
					<a class="red" data-type="mip" href="{:url('/author/' . urlencode($novel['Author']) )}" data-title="{$novel.Author}作品大全">{$novel.Author} 著</a>
                    <span class="blue">{$novel.PrimaryId | show_novel_click}人读过</span>
                    <span class="red">{eq name="$novel.Full" value="1"}完结{else/}连载中{/eq}</span>
                </div>
				<div class="bookintro">
					<mip-img class="thumbnail pull-left visible-xs fmwap" layout="fixed" width="80" height="120" popup src="{$novel.LocalImage | down_image}" alt="{$novel.Title}"></mip-img>					
					<div class="bookintromore">{$novel.Introduction}</div>
				</div>
				<div>最新章节：<a class="bookchapter" href="{:url("/chapter/" . $novel.PrimaryId . "/" . get_last_offset_sort($novel.LastChapterSort,$novel['SourceList'],$novel['LastChapterFlag']),"","html",true)}" target="_blank" title="{$novel.LastChapterTitle}">{$novel.LastChapterTitle}</a></div>
                <div class="booktime">更新时间：{$novel.UpdateTime | date="Y-m-d H:i:s"}</div>
				<div class="bookmore">
					相关作品：{author_list author="$novel.Author" limit="10"}<a href="{:url("/book/" . $author['PrimaryId'],"","html",true)}" title="{$author.Title}">{$author.Title} </a>{/author_list}
				</div>
			</div>
			<div class="clear"></div>
		</div>
		<dl class="book chapterlist">
			<div class="newlist12">
			<h2>《{$novel.Title}》最新章节</h2>
				{foreach $novel.last_update_chapter_list as $chapter}
				<dd><a href="{:url("/chapter/" . $novel['PrimaryId'] . "/" . get_offset_sort($chapter['Sort'],$novel['SourceList'],$chapter['Url']),"","html",true)}" title="{$novel.Title} {$chapter.Title}" target="_blank">{$chapter.Title}</a></dd>
				{/foreach}
			</div>
		</dl>
	</div>
	...
	<div class="content">
		<dl class="book chapterlist">
			...
			<div class="fulllistall">
				<h2>《{{$novel.Title}》全部章节目录</h2>
				<mip-showmore bottomshadow='1' maxheight='1000' animatetime='0.3' id="showmore01">
		...
		<div class="book mt10 pt10 tuijian">
			推荐阅读：
	 {random_list cid="$novel.Cid" limit="10"}
	<a data-type="mip" href="{:url("/book/" . $random['PrimaryId'],"","html",true)}" data-title="{$site_config.site_name}-{$random.Title}最新章节">{$random.Title}</a>
	{/random_list}
			<div class="clear"></div>
		</div>
    </div>
