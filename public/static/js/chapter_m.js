...
script.addEventListener('load', function(){
    layui.use(['layer', 'util'], function(){
        ...
        //更新当前章节阅读位置
        $.post(update_url, {article_id:novel_id,chapter_id:novel_chapter_id,flag:novel_flag});
        ...
        var article_id = $('body').attr('article-id') || 0;
        if (article_id > 0) {
            var visit = window.localStorage.getItem('visit_'+article_id) || 0;
            if (Math.floor(visit/8640000) !== Math.floor($.now()/8640000)) {
                $.get('/book/visit/'+article_id+'/');
                window.localStorage.setItem('visit_'+article_id, $.now());
            }
            $('#vote').click(function(){
                if (!user) return login();
                $.getJSON('/book/vote/'+article_id+'/?d=json', function(res){
                    layer.msg(res._info);
                });
            });
            $('#mark').click(function(){
                if (!user) return login();
                $.getJSON('/mark/add/'+article_id+'/?d=json', function(res){
                    layer.msg(res._info);
                });
            });
            if ($('body').attr('id') == 'read') {
                ...
                var book = JSON.parse(window.localStorage.getItem('book_'+article_id) || '{"readid":0}');
                if (book.readid > 0) {
                ...
                var chapter_id = $('body').attr('chapter-id');
                var book = {
                    name:$('#bookname').text(),
                    author:$('#author').text(),
                    readid:chapter_id,
                    readname:$('.content h1').text(),
                    readtime:$.now()
                };

                window.localStorage.setItem('book_' + article_id, JSON.stringify(book));
                ...
                $('.tabbar li').click(function(){
                    switch ($(this).index()) {
                        ...
                        case 2:
                            ...
                            break;
                        case 3:
                            $(window).attr('location', '/book/'+article_id+'.html');
                            break;
                        case 4:
                ...
                return util.fixbar({
                    ...
                    click: function(type){
                        if (type === 'bar1') {
                            ...
                            if (!user) return login();
                            $.getJSON('/mark/add/'+article_id+'/'+chapter_id+'/?d=json', function(res){
